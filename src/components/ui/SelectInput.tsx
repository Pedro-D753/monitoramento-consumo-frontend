...
export function SelectInput({ label, options, value, onSelect, error, placeholder }: SelectInputProps) {
  ...
  return (
    <View style={styles.wrapper}>
      <Typography variant="regular" size="sm" color={theme.colors.text.secondary} style={styles.label}>
        {label}
      </Typography>
      
      <TouchableOpacity 
        style={[styles.container, error ? { borderColor: theme.colors.danger.main } : {}]} 
        onPress={() => setIsVisible(true)}
        activeOpacity={0.7}
      >
        <Typography color={value ? theme.colors.text.neutral : theme.colors.text.disabled}>
      ...
      {error && <Typography variant="regular" size="xs" color={theme.colors.danger.main} style={styles.error}>{error}</Typography>}
      ...
      <Modal visible={isVisible} transparent animationType="fade" onRequestClose={() => setIsVisible(false)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setIsVisible(false)}>
          <View style={styles.modalContent}>
            <Typography variant="bold" size="lg" style={styles.modalTitle}>Selecione a Unidade</Typography>
            <FlatList
              data={options}
              keyExtractor={(item) => item.value}
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.optionItem} onPress={() => handleSelect(item.value)}>
                  <Typography color={item.value === value ? theme.colors.primary.main : theme.colors.text.primary}>
                    {item.label}
                  </Typography>
                  {item.value === value && <Feather name="check" size={18} color={theme.colors.primary.main} />}
                </TouchableOpacity>
              )}
            />
          </View>
...
const styles = StyleSheet.create({
  ...
  modalTitle: { marginBottom: theme.spacing.md, textAlign: 'center' },
  optionItem: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
